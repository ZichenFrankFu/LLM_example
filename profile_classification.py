...
class CustomModelforSequenceClassification(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        if self.type == "full":
            ...
            mean_hidden = torch.mean(last_hidden_state, dim=1)
            ...
            batch_prefix = self.prefix.expand(bsz, -1, -1)
            ...
            concat_embeddings = torch.cat([batch_prefix, input_embeddings], dim=1)
            # Hint: check torch.cat for how to concatenate the tensors

            # move the input embeddings to the gpu
            inputs_embeds = concat_embeddings.to('cuda')
            ...
            prefix_atten_mask = torch.ones(bsz, prefix_length, dtype=attention_mask.dtype).to('cuda')
            attention_mask = torch.cat([prefix_atten_mask, attention_mask], dim=1)
...
def train(mymodel, num_epochs, train_dataloader, validation_dataloader, test_dataloder, device, lr, model_name):
    ...
    if mymodel.type == "head":
        ...
        head_params = list(mymodel.classifier.parameters())
        ...
        for index, batch in tqdm(enumerate(train_dataloader)):
            ...
            with profile(activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA],
                ...
                with record_function("forward"):
                    ...
                    predictions = output['logits']
                    t_loss = loss(predictions, labels)
...
def plot(train_list, valid_list, name, finetune_method):
    ...
    plt.legend()
    plt.savefig(f'profile_cls_{name}_{finetune_method}.png')
